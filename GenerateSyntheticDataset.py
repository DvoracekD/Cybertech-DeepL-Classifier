...
def mask_to_polygon(mask, normalized=True):
    ...
    for i in range(0, len(largest_contour), 5):
        ...
        normalized_x = x / mask.shape[1] if normalized else x
        normalized_y = y / mask.shape[0] if normalized else y
...
def generate_dataset(custom_num_images=NUM_IMAGES_TO_GENERATE, segmentation=False):
    ...
    def generate_to_folder(subdir_name, n_images=NUM_IMAGES_TO_GENERATE):
        ...
        categories = [{
            ...
            "id": id,
            "name": category
        } for category, id in class_dict.items()]
        ...
        for image_num in range(n_images):
            ...
            mask = mask.reshape((*mask.shape[:2], -1))  # ensure third dimension
            file_name = "image_{}.png".format(image_num)
            image_h, image_w, _ = image.shape
            cv2.imwrite(DATASET_DIRECTORY + "images/" + subdir_name + "/" + file_name, image)
            ...
            image_entry = {
                ...
                "file_name": file_name,
                "width": image_w,
                "height": image_h,  # Add actual image height
                "license": 0,
                "flickr_url": "",
                "coco_url": "",
                "date_captured": ""
            ...
            for obj_index in range(len(bbox)):
                ...
                if segmentation:
                    ...
                    box_x, box_y, box_w, box_h, category_id = bbox[obj_index]
                    abs_box_x, abs_box_y, abs_box_w, abs_box_h = box_x * image_w, box_y * image_h, box_w * image_w, box_h * image_h
                    annotation_entry = {
                        "id": annotation_id,
                        "image_id": image_num,
                        "category_id": category_id + 1,
                        "bbox": [abs_box_x - abs_box_w / 2, abs_box_y - abs_box_h / 2, abs_box_w, abs_box_h],
                        # x,y center of box -> corner
                        "area": abs_box_w * abs_box_h,
                        "iscrowd": 0,
                        "segmentation": [list(map(float, mask_to_polygon(mask[:,:,obj_index], normalized=False)))],
                        "width": abs_box_w,
                        "height": abs_box_h
                    }
                    annotation_id += 1
        ...
        with open(DATASET_DIRECTORY + subdir_name + "_info.json", 'w') as json_file:
...
if __name__ == "__main__":
    generate_dataset(segmentation=True)
